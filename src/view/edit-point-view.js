...
${cities.map(function getCities(c) { return `<option value="${c}">${c}</option>`; })}
    ...
    <div class="event__field-group  event__field-group--time">
      ...
      <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${startDate('fd')}">
        —
        <label class="visually-hidden" for="event-end-time-1">To</label>
        <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${endDate('fd')}">
        </div>
  ...
  <section class="event__section  event__section--destination">
    ...
    ${destination.pictures.map(function getImages(picPath) { return `<img class="event__photo" src="${picPath.src}" alt="${picPath.description}">` }).join('')}
  </div>
  </div>
    </section>
      </section >
